...
def processing_presense_reestr():
    ...
    try:
        ...
        messagebox.showerror('Артемида 1.7',
        ...
        messagebox.showerror('Артемида 1.7', f'Выберите файл с данными и конечную папку')
    except PermissionError:
        messagebox.showerror('Артемида 1.7', f'Закройте файлы с созданными раньше отчетами!!!')
    except FileNotFoundError:
        messagebox.showerror('Артемида 1.7', f'Проверьте наличие указанных файлов')
    except MemoryError:
        messagebox.showerror('Артемида 1.7', f'Слишком большая таблица!!!\nПроверьте размер таблицы!!!\nНажмите CTRL+END для проверки'
                                             f'Пересоздайте файл без пустых строк и столбцов')
    else:
        messagebox.showinfo('Артемида 1.7', 'Работа программы успешно завершена!!!')
...
def prepare_column_purpose_category(df,name_columns):
    ...
    try:
        ...
        df[name_columns] = df[name_columns].astype(str)
            ...
            messagebox.showerror('Артемида 1.7',f'Не найдена колонка {e.args} Проверьте файл на наличие этой колонки')
    except ValueError as e:
        messagebox.showerror('Артемида 1.7', f'Возникла ошибка при обработке значения {e.args}\n'
